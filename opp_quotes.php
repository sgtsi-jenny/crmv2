<?php
    ...
    $data="";
   $data=$con->myQuery("SELECT title,q.description,date_uploaded,CONCAT(users.last_name, ', ', users.first_name) AS uname, user_name, document, q.id FROM quotes q INNER JOIN opportunities ON q.opportunity_name=opportunities.id INNER JOIN users ON q.user_name=users.id WHERE q.is_deleted=0 AND q.opportunity_name=? AND users.user_type_id=?",array($opp['id'],$_SESSION[WEBAPP]['user']['id']))->fetchAll(PDO::FETCH_ASSOC);
                          ...
                          <tbody>
                                            ...
                                            ?>
                                                <tr>
                                                    ...
                                                    ?>
                                                    <td align="center">
                                                        <a href='opp_quotes.php?id=<?php echo $opp['id']?>&quote_id=<?php echo $row['id']?>' class='btn btn-sm btn-success'><span class='fa fa-pencil'></span></a>
